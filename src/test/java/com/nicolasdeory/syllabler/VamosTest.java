...
public class VamosTest {

    private static final String YAML_RESOURCE = "vamos_test.json";
    private static final String[] WORD_EXPECTATIONS = {
        "syllables"
    };

    private Rule rule = new Vamos();
    ...
    public static Iterable<Object[]> data() throws Exception {
        Gson gson = new Gson();
        InputStream resourceAsStream = VamosTest.class.getClassLoader().getResourceAsStream(YAML_RESOURCE);
        Map<String, Map<String, Object>> testCases = gson.fromJson(new InputStreamReader(resourceAsStream), Map.class);
        Stream<Object[]> stream = testCases.entrySet().stream().map(VamosTest::entryData);
