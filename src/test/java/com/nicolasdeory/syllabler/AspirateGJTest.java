...
public class AspirateGJTest {

    private static final String YAML_RESOURCE = "aspirate_gj.json";
    private static final String[] WORD_EXPECTATIONS = {
        "syllables"
    };

    private Rule rule = new AspirateGJ();
    ...
    public static Iterable<Object[]> data() throws Exception {
        Gson gson = new Gson();
        InputStream resourceAsStream = AspirateGJTest.class.getClassLoader().getResourceAsStream(YAML_RESOURCE);
        Map<String, Map<String, Object>> testCases = gson.fromJson(new InputStreamReader(resourceAsStream), Map.class);
        Stream<Object[]> stream = testCases.entrySet().stream().map(AspirateGJTest::entryData);
