...
public class RemoveAdoIdoTest {

    private static final String YAML_RESOURCE = "remove_ado.json";
    private static final String[] WORD_EXPECTATIONS = {
        "syllables"
    };

    private Rule rule = new RemoveAdoIdo();
    ...
    public static Iterable<Object[]> data() throws Exception {
        Gson gson = new Gson();
        InputStream resourceAsStream = RemoveAdoIdoTest.class.getClassLoader().getResourceAsStream(YAML_RESOURCE);
        Map<String, Map<String, Object>> testCases = gson.fromJson(new InputStreamReader(resourceAsStream), Map.class);
        Stream<Object[]> stream = testCases.entrySet().stream().map(RemoveAdoIdoTest::entryData);
